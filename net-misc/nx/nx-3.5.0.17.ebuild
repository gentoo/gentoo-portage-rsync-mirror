...
# $Header: /var/cvsroot/gentoo-x86/net-misc/nx/nx-3.5.0.17.ebuild,v 1.1 2013/01/08 10:47:02 voyageur Exp $
...
src_prepare() {
	...
	# Drop force -O3 and run autoreconf in all neeed folders
	for i in nxcomp nxcompext nxcompshad nxproxy; do
		cd "${S}"/${i}
		sed -e "s/-O3//" -i configure.in ||  die "$i configure sed failed"
		eautoreconf ${i}
		cd "${S}"
	done
