...
# $Header: /var/cvsroot/gentoo-x86/dev-python/chaco/chaco-4.3.0-r1.ebuild,v 1.1 2013/04/15 09:45:43 idella4 Exp $
...
copy_tests() {
	cd "${BUILD_DIR}"/lib
	mkdir -p ${PN}/{shell,scales}
	cp -a "${S}"/${PN}/tests/ ${PN}/
	cp -a "${S}"/${PN}/shell/tests/ ${PN}/shell/
	cp -a "${S}"/${PN}/scales/tests/ ${PN}/scales/
}

python_test() {
	# skirt tests until fixed; https://github.com/enthought/chaco/issues/112
	copy_tests || die
	VIRTUALX_COMMAND="nosetests -e test_estimate_default_scale* \
		-e test_tfrac_hours_04* -e test_tfrac_hours_05* \
		-e test_microsecond* -I border_test_case*" virtualmake
