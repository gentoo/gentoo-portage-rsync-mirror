...
# $Header: /var/cvsroot/gentoo-x86/www-servers/monkeyd/monkeyd-1.4.0.ebuild,v 1.1 2014/02/05 13:46:33 blueness Exp $
...
src_prepare() {
	...
	rm -rf "${S}"/deps
	epatch "${FILESDIR}"/${PN}-1.4.0-use-system-jemalloc.patch
...
src_configure() {
	...
	use uclibc && myconf+=" --uclib-mode"
	use musl && myconf+=" --musl-mode"
