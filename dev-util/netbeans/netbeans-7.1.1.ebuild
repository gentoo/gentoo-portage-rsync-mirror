...
# $Header: /var/cvsroot/gentoo-x86/dev-util/netbeans/netbeans-7.1.1.ebuild,v 1.3 2012/05/07 06:34:05 phajdan.jr Exp $
...
L10N_URL="http://dev.gentoo.org/~fordfrog/distfiles/netbeans-l10n-7.1.1-20120214091350.tar.bz2"
...
src_install() {
	if [ -n "${NBLOCALES}" ] ; then
		...
		for cluster in apisupport cnd dlight enterprise ergonomics groovy harness ide java javacard javafx mobility php platform profiler websvccommon ; do
			if [ -d "${cluster}" ] ; then
